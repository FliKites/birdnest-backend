...
async function storeData(drone, pilot) {
  try {
    const existingDrone = await client.query(
    ...
    const distance = Math.round(
      Math.sqrt(
        Math.pow(drone.positionX - 250000, 2) +
          Math.pow(drone.positionY - 250000, 2)
      ) / 1000
    );
    ...
    if (existingDrone.rows.length > 0) {
      ...
      await client.query(
        ...
        [position_x, position_y, distance, serial_number]
      );
    } else {
      ...
      await client.query(
        `INSERT INTO drones (
                  ...
                  phone_number,
                  email
              ) VALUES ($1, $2, $3, $4, to_timestamp($5,'YYYY-MM-DD"T"HH24:MI:SS.MS"Z"'), now(), $6, $7, $8, $9, $10)`,
        [
          serial_number,
          position_x,
          position_y,
